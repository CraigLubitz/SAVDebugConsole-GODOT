...
#		print("here")
	scrollcontainer = $Control/ScrollContainer
	vbox = $Control/ScrollContainer/VBoxContainer
	
	# i personally thought it looked nicer when made smaller for desktops
	...
	$Control/LogcatClear.visible = true;
	$Control/PrintTestLogEntry.visible = false;
...
func _process(delta):
	...
	while thiscount < console_entries_max and log_file_valid and log_file.get_position() < log_file.get_length():
		...
		if vbox.get_child_count() > console_entries_max:
		...
		# adjust the font
		RTL.add_theme_font_size_override("normal_font_size", 45)
		# add the entry
		RTL.text = entry
